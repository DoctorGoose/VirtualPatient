...
function ReinfectionModel!(du, u, p, t, state_history::StateHistory)
...
function tmap_ReinfectionModel(tspan, y0, param_sets)
    ...
    sols = ThreadsX.map(params -> solve_ReinfectionModel(tspan, y0, params), param_sets)
    return length(sols) == 1 ? sols[1] : sols
end

function solve_ReinfectionModel(tspan, y0, params)
    state_history = StateHistory(y0)
    wrapper!(du, u, p, t) = ReinfectionModel!(du, u, p, t, state_history)
