...
@everywhere using CUDA  # Added for CUDA support

# Retrieve command-line arguments
if length(ARGS) >= 2
    blas_threads = parse(Int, ARGS[1])
    procs = parse(Int, ARGS[2])
else
    blas_threads = 1
    procs = 1
end
...
@everywhere function LCTModel_cuda!(du, u, p, t)
    @inbounds begin
        # Use views for accessing GPU array slices or elements
        T = u[1]               # Element access (u[1]) is disallowed on GPU, so we'll fix this next
        I1 = u[2]
        I2 = u[3]
        V = u[4]
        E = u[5]
        ...
        # Access parameters in p similarly
        beta = p[1]
        k = p[2]
        delta = p[3]
        delta_E = p[4]
        ...
        # Use GPU-friendly arithmetic, avoiding any transfer back to CPU
        tau_inv = 1f0 / tau
        xi_tau_inv = xi * tau_inv
        delta_E_term = delta_E * E * I2 / (K_delta_E + I2)
        ...
        du[3] = k * I1 - delta * I2 - delta_E_term
        du[4] = p_param * I2 - c * V
        du[5] = xi_tau_inv * u[end] - d_E * E
        du[6] = d_E * E  # Lung T cells

        # Delayed variables
        du[7] = tau_inv * (a * I2 - u[7])
        du[8:end] .= tau_inv .* (u[7:end-1] .- u[8:end])
    end
    return nothing
end
...
# Function to solve ODEs using CUDA
function solve_LCTModel_cuda(tspan, y0, params)
    # Transfer data to GPU
    y0_gpu = CuArray(y0)
    params_gpu = CuArray(params)

    prob = ODEProblem(LCTModel_cuda!, y0_gpu, tspan, params_gpu)
    sol_gpu = suppress_warnings(() -> solve(
        prob,
        Tsit5(),                   # GPU-compatible solver
        reltol = 1e-7,             # Relative tolerance
        abstol = 1e-4,             # Absolute tolerance
        save_everystep = true,     # Save at every integration step
        save_start = true,         # Save initial condition
    ...
    # Instead of modifying `sol_gpu`, we extract the data
    sol_t_cpu = collect(Array(sol_gpu.t))
    sol_u_cpu = [Array(u) for u in sol_gpu.u]
    # Create a new ODESolution with CPU arrays
    sol = ODESolution(sol_gpu.prob, sol_u_cpu, sol_t_cpu, sol_gpu.k, sol_gpu.dense, sol_gpu.destats)
    return sol
end
...
# CUDA implementation
function cuda_LCTModel(tspan, y0, param_sets)
    sols = [solve_LCTModel_cuda(tspan, y0, params) for params in param_sets]
    return sols
end
...
function test_methods_on_param_sets(param_sets, tspan, y0; profile=true)
    methods = Dict(
        ...
        #:pmap   => pmap_LCTModel,
        #:tmap   => tmap_LCTModel,
        :cuda   => cuda_LCTModel    # Added CUDA method
    )
